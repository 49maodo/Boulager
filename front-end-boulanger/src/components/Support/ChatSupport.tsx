...
export function ChatSupport() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch(
        addAiMessage("Une erreur s'est produite lors de l'envoi du message."),
      );
    }
  };
  ...
  const MessageBubble = ({ message }: { message: any }) => {
    ...
    return (
      ...
      >
        <div
          className={`flex items-start max-w-[70%] ${isOwnMessage ? "flex-row-reverse" : "flex-row"}`}
        >
          ...
          <div
            className={`rounded-lg p-3 ${isOwnMessage ? "bg-primary text-primary-foreground" : isAIMessage ? "bg-blue-50 border border-blue-200 dark:bg-blue-950 dark:border-blue-800" : "bg-muted"}`}
          >
            <p className="text-sm">{message.contenu}</p>
            <div
              className={`flex items-center justify-between mt-1 text-xs ${isOwnMessage ? "text-primary-foreground/70" : "text-muted-foreground"}`}
            >
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Cliquez sur une question pour l'envoyer rapidement
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-lg">
                {isClient ? "Conversation avec le support" : "Messages clients"}
              </CardTitle>
              <CardDescription>
                {isClient
                  ? "Notre équipe et notre IA vous répondent rapidement"
          ...
          <div className="border-t p-4 shrink-0 bg-background">
            ...
            <form onSubmit={handleSendMessage} className="flex space-x-2">
              <Input
                ...
                placeholder={
                  isClient ? "Tapez votre message..." : "Répondre au client..."
                }
                disabled={isSending}
                className="flex-1"
              />
            ...
            {isClient && (
              <div className="mt-2 text-xs text-muted-foreground">
                Notre IA et notre équipe vous répondent généralement en moins de
                5 minutes pendant les heures d'ouverture.
              </div>
            )}
          </div>
        </CardContent>
